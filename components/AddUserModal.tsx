        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50" onClick={onClose}>
            <div className="bg-white rounded-xl border border-border shadow-lg p-6 w-full max-w-md m-4 transform transition-all duration-300" onClick={e => e.stopPropagation()}>
                ...
                <form className="space-y-4" onSubmit={handleSubmit}>
                    <Input id="user-email" label="Email" type="email" value={email} onChange={e => setEmail(e.target.value)} required variant="admin" />
                    <Input id="user-password" label="Password" type="password" value={password} onChange={e => setPassword(e.target.value)} required variant="admin" />
                    <Input id="user-confirm-password" label="Confirm Password" type="password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} required variant="admin" />
